...
from lerf.data.utils.pyramid_embedding_dataloader import \
...
@dataclass
class InHandDataManagerConfig(VanillaDataManagerConfig):
    _target: Type = field(default_factory=lambda: InHandDataManager)
    patch_tile_size_range: Tuple[int, int] = (0.05, 0.5)
    patch_tile_size_res: int = 7
    patch_stride_scaler: float = 0.5


class InHandDataManager(VanillaDataManager):  # pylint: disable=abstract-method
    ...
    config: InHandDataManagerConfig
