...
from nerfstudio.data.dataparsers.base_dataparser import (
    DataParser, DataParserConfig, DataparserOutputs, RobotDataparserOutputs)
...
class Nerfstudio(DataParser):
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        for frame in frames:
            ...
            if "robot_mask_path" in frame:
                robot_mask_filepath = Path(frame["robot_mask_path"])
                robot_mask_fname = self._get_fname(
                    robot_mask_filepath,
                    data_dir,
                    downsample_folder_prefix="robot_masks_",
                )
                robot_mask_filenames.append(robot_mask_fname)
        ...
        robot_mask_filenames = (
            [robot_mask_filenames[i] for i in indices]
            if len(robot_mask_filenames) > 0
            else []
        )
        depth_filenames = (
        ...
        dataparser_outputs = RobotDataparserOutputs(
            image_filenames=image_filenames,
            cameras=cameras,
            scene_box=scene_box,
            mask_filenames=mask_filenames if len(mask_filenames) > 0 else None,
            robot_mask_filenames=robot_mask_filenames if len(robot_mask_filenames) > 0 else None,
